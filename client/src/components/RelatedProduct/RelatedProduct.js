            ...
            <Grid container spacing={4}>
                {data?.map((perfume) => (
                    <Grid item xs={6} sm={4} md={3} lg={3} key={perfume._id}>
                        <a
                            href={`/${i18n.language}/${perfume.nameEn}/${perfume._id}`}
                            onClick={(e) => {
                                // href for clicking on the left side
                                e.preventDefault(); // Prevent default link behavior
                                handleNavigateProductDetails(perfume); // Trigger navigation
                            }}
                            target="_blank" // Allow opening in new tab
